...
export function ListFilters({
  ...
  const hasActiveFilter = Object.values(defaults).some((v) => v);

  return (
    <details
      open={hasActiveFilter}
      className="rounded-2xl border border-hairline [&_summary::-webkit-details-marker]:hidden"
    >
      <summary className="cursor-pointer select-none px-4 py-3 text-sm font-medium text-ink flex items-center gap-2">
        <span>필터{hasActiveFilter ? " (적용됨)" : ""}</span>
      </summary>
      <form
        method="get"
        className="flex flex-wrap items-end gap-3 border-t border-hairline p-4"
      >
        <div>
