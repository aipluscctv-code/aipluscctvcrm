...
- **사진 배치 마커**: 사용자 메시지에 적힌 "사진 개수"만큼 사진이 순서대로 주어집니다(1번째 사진=사진1, 2번째=사진2...). 본문에서 그 사진을 보여주면 좋을 문단 바로 뒤에 \`[사진N]\` 형식으로 마커를 독립된 줄에 삽입하세요(N은 숫자, 대괄호와 "사진" 사이 공백 없이). 주어진 사진 중 최소 3장 이상을 실제로 본문 각기 다른 위치에 배치하고, 마커 주변 문단이 그 사진 내용과 실제로 연결되도록 쓸 것(예: 카메라 설치 위치를 설명한 문단 뒤에 그 위치가 찍힌 사진 번호)
